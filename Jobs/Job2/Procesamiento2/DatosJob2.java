 ...
 private Connection conexion;//Creamos una variable privada para conectarnos
    ...
    public ResultSet consultar(String sql) {
	 ...
	 public static void main(String[] args) throws Exception {
		DatosJob2 r = new DatosJob2();
		r.conectar();//Conexion
		  Configuration conf = new Configuration();
		  FileSystem hdfs = FileSystem.get(conf);
		  Path path = new Path("/user/cloudera/job2/output/part-r-00000");
		BufferedReader br = new BufferedReader(new InputStreamReader (hdfs.open(path)));//Carga los datos
            String line;
		
            line=br.readLine();//Lee la linea
		String[] partes = line.split(",");//Separa y se asignan las variables
		//System.out.println(partes[0]);
		String palabra = partes[0];
		String url = partes[1];
		String titulo = partes[2];
		String numero = partes[3].replace("	","");
		
		
		int conteo = Integer.parseInt(numero);
		String sql = "Insert into job2 (Palabra, Url, Conteo, Titulo) Values('" + palabra + "','" + url + "'," + conteo + ",'" + titulo + "');";//Sql del insert
		...
		url = partes[1];
		titulo = partes[2];
		numero = partes[3].replace("	","");
		
		
		conteo = Integer.parseInt(numero);
			
		sql = "Insert into job2 (Palabra, Url, Conteo, Titulo) Values('" + palabra + "','" + url + "'," + conteo + ",'" +  titulo + "');";
		System.out.println(sql);
		r.ejecutar(sql);
